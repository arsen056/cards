...
export const packsReducer = (
  state: PacksStateType = initState,
...
export const setPackName = (packName: string) =>
  ({ type: 'PACKS/SET_PACK_NAME', packName } as const)
...
export const addPackAC = (newCardsPack: PackType) =>
  ({ type: 'PACKS/ADD-PACKS', newCardsPack } as const)
...
export const getCardsPackAC = (data: PacksResponseType) =>
  ({ type: 'PACK/GET_CARDS_PACK', data } as const)
export const setIsMyPacks = (isMyPacks: boolean) =>
  ({ type: 'PACK/IS_MY_PACK', isMyPacks } as const)
export const setResetFilters = (max: number) =>
  ({
    type: 'PACK/SET_RESET_FILTERS',
    filters: {
      packName: '',
      user_id: '',
      min: 0,
